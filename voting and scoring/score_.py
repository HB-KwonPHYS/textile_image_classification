...
from CNN_mod import CNN_mod, CNN_mod_deep, MonoCNN_deep, MonoCNN
...
def parse_arguments():
    ...
    parser.add_argument('--model', default='mobilenetv2', help='mobilenetv2 | xception | deit_tiny | cnn | cnn_deep | mono_cnn | mono_cnn_deep')
    parser.add_argument('--test_data_folder', default='textile_origin_split/Textile_center_12/test', help = 'file_folder')
    parser.add_argument('--load_ckpt', default='checkpoint/mobilenetv2_six_6_lr0.0001.pth', help = 'checkpoint')
...
class select_model(nn.Module):
    def __init__(self, num_cls, model_name):
        ...
        elif model_name == 'mono_cnn':
            self.model = MonoCNN()
        elif model_name == 'mono_cnn_deep':
            self.model = MonoCNN_deep()
        else:
...
def transform_test():
    transform = transforms.Compose([
        ...
        #transforms.Grayscale(num_output_channels=1),  # 이미지를 1채널(grayscale)로 변경
        transforms.ToTensor(),
